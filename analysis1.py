...
def analyze_single_image(filename, minPxs, calibrator, Histmin, Histmax, rois):
    # Code to analyze a single image
    # ... (Same as the code inside the for loop in the original function)
...
# Plantcv based colorspaces and gray image
    ...
    plt.close()
    # pic a color space and call it gray
    # replace lab with hsv for hsv color pixels
    gray = pcv.rgb2gray_lab(rgb_img=img, channel='a')
    # gray=a
    plt.imshow(gray)
    plt.close()
    # hist = pcv.visualize.histogram(img=gray, bins=50)
    # hist = cv2.calcHist([gray], [0], None, [255], [0,255])
    # plt.plot(hist)

    # try adjusting for the cv2
    # Hist=hist

    Hist = cv2.calcHist([gray], [0], None, [256], [0, 256])
    # Threshold = Histmin + np.argmin(Hist[Histmin:Histmax])#this calue is 147 while threshold=100. Try ti equalize them below
    #Threshold=threshold #
 ...
 # Define individual plants as objects
    ...
    #object_count = len(rois_with_objects)     
    # Calculate germination percent
    germination_percent = object_count / 4
    # Count objects that partially touch the line (up to 4 objects)
    #partial_touch_count = min(4, Objects)
    #print(Objects)
    ...
    plt.plot(Hist), plt.yscale('log'), plt.savefig(str(fname)+"_histogram.png"), plt.close()#replace Hist with hist
    ...
    CFI_img_f64 = np.float64(gray) #For RGB
    ...
    # Experienced ZeroDivisionError (error handling 'except' is needed to avoid script stopping)
...
# Calculate standard deviation:
    ...
    #plt.imshow(img)
    #plt.show()
    #return filename, Threshold, pixel_number, Avg_CFI, Std_CFI, area, Objects, AvgArea, Std_Area
...
def PxEx(minPxs, calibrator, imageformat, Histmin, Histmax, csvname, directory):
    ...
    for fdx, filename in enumerate(fileList):
        try:
            ...
            # Write default values (zeros or NAs) for the problematic image to the CSV file
            with open(csvname, "a", newline='') as csvfile: 
                writer = csv.writer(csvfile)
...
# Parameter 3) This is the format of your image. You can change jpg (default now) as any other format.
# Parameter 4 and 5) They are the minimum and maximum values for adjusting threshold. 
#           e.g.: Threshold = 20 + np.argmin(Hist[20:90]); 20 is an example minimum value when the background is dark
...
PxEx(80, 4.777777778, '.png', 109, 130, r'C:\Users\mi79012\OneDrive - University of Georgia\Pi\pi238\Upload\study1\result.csv', r'C:\Users\mi79012\OneDrive - University of Georgia\Pi\pi238\Upload\study1')
